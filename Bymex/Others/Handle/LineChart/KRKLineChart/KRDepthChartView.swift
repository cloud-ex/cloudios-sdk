...
/// - bid: 买方深度
/// - ask: 卖方深度
public enum KRKDepthChartItemType {
    case bid
    case ask
}
...
/// - left: 左边
/// - right: 右边
public enum KRKDepthChartOnDirection {
    case left
    case right
}
...
@objc public protocol CHKDepthChartDelegate: class {
    ...
    /**
     ...
     - returns:
     */
    @objc func depthChart(chart: KRDepthChartView, labelOnYAxisForValue value: CGFloat) -> String
    ...
    /// - Returns: 开始显示的值
    @objc optional func baseValueForYAxisInDepthChart(in depthChart: KRDepthChartView) -> Double
    ...
    /// - Parameter depthChart: 图表
    /// - Returns: 增量
    @objc optional func incrementValueForYAxisInDepthChart(in depthChart: KRDepthChartView) -> Double
    ...
    /**
     ...
     - returns:
     */
    @objc optional func depthChart(chart: KRDepthChartView, labelOnXAxisForIndex index: Int) -> String
    
    /**
     完成绘画图表
     
     */
    @objc optional func didFinishDepthChartRefresh(chart: KRDepthChartView)
    ...
    ///
    /// - returns:
    @objc optional func widthForYAxisLabelInDepthChart(in depthChart: KRDepthChartView) -> CGFloat
    ...
    ///   - point: 点击的位置
    ///   - item: 数据对象
    @objc optional func depthChart(chart: KRDepthChartView,Selected item: KRKDepthChartItem,At point:CGPoint)
    ...
    /// - Parameter chart: 图表
    /// - Returns: 返回自定义的高度
    @objc optional func heightForXAxisInDepthChart(in depthChart: KRDepthChartView) -> CGFloat
    ...
    /**
     ...
     - returns:
     */
    @objc func depthChartOfDecimal(chart: KRDepthChartView) -> Int
    ...
    /**
     ...
     - returns:
     */
    @objc func depthChartOfVolDecimal(chart: KRDepthChartView) -> Int
    ...
    /**
     ...
     - returns:
     */
    @objc optional func depthChartShowItemView(chart: KRDepthChartView,Selected item: KRKDepthChartItem) -> UIView?
    ...
    /**
     ...
     - returns:
     */
    @objc optional func depthChartTagView(chart: KRDepthChartView,Selected item: KRKDepthChartItem) -> UIView?
    
}

open class KRDepthChartView: UIView {
    ...
    open var yAxis: KRYAxis = KRYAxis()                           //Y轴参数
    open var xAxis: KRXAxis = KRXAxis()                             //X轴参数
    ...
    /// 点击选中的标记图形
    var selectedTagGraphslayer:KRShapeLayer?
    var selectedTagCircleCnterlayer:KRShapeLayer?
    
    /// 点击选中的数据点view
    var selectedItemInfoLayer:KRShapeLayer?
    ...
    /// 买方深度数据
    open var bidItems = [KRKDepthChartItem]()
    
    /// 卖方深度数据
    open var askItems = [KRKDepthChartItem]()
    ...
    open var style: KRKLineChartStyle! {           //显示样式
        didSet {
            ...
            self.enableLongPress = self.style.enableLongPress
    ...
    fileprivate func initUI() {
        ...
        let long = UILongPressGestureRecognizer(target: self, action: #selector(doLongPressAction(_:)))
        long.delegate = self
    ...
    ///
    /// - Parameter item: 数据集合
    fileprivate func computeDepthValue(for items: [KRKDepthChartItem], type: KRKDepthChartItemType) {
    ...
    func setSelectedIndexByPoint(_ point: CGPoint) {
        ...
        selectedTagGraphslayer?.isHidden = false
        selectedItemInfoLayer?.isHidden = false
        ...
        // 点击选中的item
        var selectedItem:KRKDepthChartItem?
        // 判断买单还是卖单
...
extension KRDepthChartView {
    ...
    /**
     ...
     - parameter section:
     */
    func drawTagGraph(point:CGPoint,item:KRKDepthChartItem){
        if self.selectedTagGraphslayer == nil{
            self.selectedTagGraphslayer = KRShapeLayer()
            self.selectedTagCircleCnterlayer = KRShapeLayer()
        }
        ...
        if let view = self.delegate?.depthChartTagView?(chart: self, Selected: item){
            ...
            if item.type == .bid{
                ...
                self.selectedTagGraphslayer!.backgroundColor = UIColor.clear.cgColor
                self.selectedTagGraphslayer!.borderColor = self.bidColor.stroke.cgColor
                self.selectedTagCircleCnterlayer?.backgroundColor = self.bidColor.stroke.cgColor
            }else{
                //self.selectedTagGraphslayer!.backgroundColor = self.askColor.fill.cgColor
                self.selectedTagGraphslayer!.backgroundColor = UIColor.clear.cgColor
                self.selectedTagGraphslayer!.borderColor = self.askColor.stroke.cgColor
                self.selectedTagCircleCnterlayer?.backgroundColor = self.askColor.stroke.cgColor
            }
            ...
            //圆心
            let r: CGFloat = 3
            self.selectedTagCircleCnterlayer?.frame = CGRect.init(x: selectedTagGraphslayer!.frame.width/2-r, y: selectedTagGraphslayer!.frame.height/2-r, width: 2*r, height: 2*r)
            self.selectedTagCircleCnterlayer?.cornerRadius = r
        }
        self.drawLayer.addSublayer(self.selectedTagGraphslayer!)
        self.selectedTagGraphslayer?.addSublayer(self.selectedTagCircleCnterlayer!)
    ...
    /**
     ...
     - parameter section:
     */
    func drawItemInfo(point: CGPoint,item:KRKDepthChartItem){
        // 长宽高
        ...
        if let view = self.delegate?.depthChartShowItemView?(chart: self, Selected: item) {
            if self.selectedItemInfoLayer == nil{
                self.selectedItemInfoLayer = KRShapeLayer()
            }
            ...
            // 价格
            let pricelayer = KRTextLayer()
            pricelayer.string = item.value.kr_toString(maxF:self.decimal)//String(Double(iteme.value))
            ...
            if amount >= 1000{
                ...
                amountStr = amount.kr_toString(maxF:self.numDecimal)//String(Double(amount))
            }
    ...
    func drawChart() -> [(CGRect, String)]{
        ...
        if self.bidChartOnDirection == .right{
            ...
            let asksValue = self.askItems[self.askItems.endIndex - 1].value.kr_toString()
            ...
            xAxisToDraw.append((sell1Rect, self.askItems[self.askItems.endIndex - 1].value.kr_toString()))
            ...
            xAxisToDraw.append((buy1Rect, self.bidItems[self.bidItems.endIndex - 1].value.kr_toString()))
            ...
            xAxisToDraw.append((bidsRect, self.bidItems[self.bidItems.endIndex - 1].value.kr_toString()))
            ...
            let bidsRect = CGRect(x: bidsX, y: bidsY, width: 60, height: 18)
            xAxisToDraw.append((bidsRect, self.bidItems.first?.value.kr_toString() ?? "--"))
            ...
            let buy1Rect = CGRect(x: buy1X, y: buy1Y, width: 60, height: 18)
            var str1: String! = "--"
            if self.bidItems.count > 1 {
                str1 = self.bidItems[self.bidItems.endIndex - 1].value.kr_toString()
            }else if self.bidItems.count > 0 {
                str1 = self.bidItems.last?.value.kr_toString()
            }
            xAxisToDraw.append((buy1Rect, str1))
            
            let sell1X = self.bounds.origin.x + self.padding.left + CGFloat(startIndex) * plotWidth + 8
            let sell1Y = bidsY
            let sell1Rect = CGRect(x: sell1X, y: sell1Y, width: 60, height: 18)
            xAxisToDraw.append((sell1Rect, self.askItems.first?.value.kr_toString() ?? "--"))
            ...
            let asksRect = CGRect(x: asksX, y: asksY, width: 60, height: 18)
            var str2: String! = "--"
            if self.askItems.count > 1 {
                str2 = self.askItems[self.askItems.endIndex - 1].value.kr_toString()
            }else if self.askItems.count > 0 {
                str2 = self.askItems.last?.value.kr_toString()
            }
            
            xAxisToDraw.append((asksRect, str2))
    ...
    ///   - strokeColor: 线条颜色
    ///   - fillColor: 填充颜色
    func drawDepthChart(items: [KRKDepthChartItem],
                        startIndex: Int,
                        strokeColor: UIColor,
                        fillColor: UIColor,
...
// MARK: - 手势操作
extension KRDepthChartView: UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        ...
        case is UITapGestureRecognizer:
            return true//self.enableTap
        case is UILongPressGestureRecognizer:
            return self.enableLongPress
        default:
            return false
        }
    }
...
//        self.setSelectedIndexByPoint(point)
        ...
        selectedTagGraphslayer?.isHidden = true
        selectedItemInfoLayer?.isHidden = true
        
        self.selectedPoint = .zero
        self.setSelectedIndexByPoint(self.selectedPoint)
    }

...
/**
 ...
 - Equal: 相等
 */
public enum KRChartItemTrend {
    case up
    case down
    case equal
}

open class KRRangeModle: NSObject {
    open var range: Int = 0
    open var maxRange: Int = 0
    open var minRangeWhenRightSpace: Int = 0
}

/**
 *  数据元素
 */
open class KRChartItem: NSObject {
    ...
    open var trend: KRChartItemTrend {
...
open class KRChartModel {
    ...
    open var datas: [KRChartItem] = [KRChartItem]()               //数据值
 ...
 *  线点样式模型
 */
open class KRLineModel: KRChartModel {
 ...
 *  蜡烛样式模型
 */
open class KRCandleModel: KRChartModel {
    ...
    open override func drawSerie(_ startIndex: Int, endIndex: Int) -> CAShapeLayer {
        ...
        for i in stride(from: startIndex, to: endIndex, by: 1) {
            ...
            if self.key != KRSeriesKey.candle {
                //不是蜡烛柱类型，要读取具体的数值才绘制
            ...
            lastCandleLayerXAddW = (ix + plotPadding) + (plotWidth - 2 * plotPadding)
 ...
 *  交易量样式模型
 */
open class KRColumnModel: KRChartModel {
    ...
    open override func drawSerie(_ startIndex: Int, endIndex: Int) -> CAShapeLayer {
        ...
        for i in stride(from: startIndex, to: endIndex, by: 1) {
            ...
            if self.key != KRSeriesKey.volume {
                //不是蜡烛柱类型，要读取具体的数值才绘制
                if self[i].value == nil {       //读取的值
                    continue  //无法计算的值不绘画
                }
 ...
 *  交易量样式模型
 */
open class KRBarModel: KRChartModel {
 ...
 *  圆点样式模型
 */
open class KRRoundModel: KRChartModel {
...
// MARK: - 扩展公共方法
public extension KRChartModel {
    
    /**
     绘画最大值
     */
    func drawGuideValue(value: String, section: KRSection, point: CGPoint, trend: KRChartItemTrend) -> CAShapeLayer {
        
        let guideValueLayer = CAShapeLayer()
        
        let fontSize = value.kr_sizeWithConstrained(section.labelFont)
        let arrowLineWidth: CGFloat = 4
        var isUp: CGFloat = -1
        var isLeft: CGFloat = -1
        var tagStartY: CGFloat = 0
        var isShowValue: Bool = true        //是否显示值，圆形样式可以不显示值，只显示圆形
        var guideValueTextColor: UIColor = UIColor.kr_hex(0xF5F5F8)//UIColor.white              //显示最大最小的文字颜色
        //判断绘画完整时是否超过界限
        ...
        case let .line(color):
            ...
            guideValueTextColor = color
            arrowPath.move(to: CGPoint(x: point.x, y: point.y))
            arrowPath.addLine(to: CGPoint(x: point.x + arrowLineWidth * isLeft * 3, y: point.y))
        ...
        if isShowValue {
            ...
            //计算画文字的位置
            var point = CGPoint(x: maxPriceStartX, y: maxPriceStartY)
            switch self.ultimateValueStyle {
            case .line(_):
                point = CGPoint(x: maxPriceStartX+arrowLineWidth * isLeft, y: maxPriceStartY - arrowLineWidth * isUp * 2)
            default:
                break
            }
            let textSize = value.kr_sizeWithConstrained(section.labelFont)
            
            //画最大值数字
            let valueText = KRTextLayer()
...
extension KRChartModel {
    ...
    //生成一个点线样式
    class func getLine(_ color: UIColor, title: String, key: String, lineWidth: CGFloat = 1) -> KRLineModel {
        let model = KRLineModel(upStyle: (color, true), downStyle: (color, true),
                                titleColor: color)
        model.title = title
        model.key = key
    ...
    class func getCandle(upStyle: (color: UIColor, isSolid: Bool),
                         downStyle: (color: UIColor, isSolid: Bool),
                         titleColor: UIColor,
                         key: String = KRSeriesKey.candle) -> KRCandleModel {
        let model = KRCandleModel(upStyle: upStyle, downStyle: downStyle,
    ...
    class func getVolume(upStyle: (color: UIColor, isSolid: Bool),
                         downStyle: (color: UIColor, isSolid: Bool),
                         key: String = KRSeriesKey.volume) -> KRColumnModel {
        let model = KRColumnModel(upStyle: upStyle, downStyle: downStyle,
    ...
    class func getBar(upStyle: (color: UIColor, isSolid: Bool),
                      downStyle: (color: UIColor, isSolid: Bool),
                      titleColor: UIColor, title: String, key: String) -> KRBarModel {
        let model = KRBarModel(upStyle: upStyle, downStyle: downStyle,
                               titleColor: titleColor)
        model.title = title
        model.key = key
        return model
    ...
    class func getRound(upStyle: (color: UIColor, isSolid: Bool),
                        ...
                        key: String) -> KRRoundModel {
        let model = KRRoundModel(upStyle: upStyle, downStyle: downStyle,
...
extension KRChartModel {
    ...
    public subscript (index: Int) -> KRChartItem {
        var value: CGFloat?

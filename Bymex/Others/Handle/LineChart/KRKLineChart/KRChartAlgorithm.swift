...
public protocol KRChartAlgorithmProtocol {
    
    /// 实现该算法的处理
    /// 通过传入一个基本的K线数据模型集合给委托者，完成指标算法计算，
    /// 并把结果记录到KRChartItem的extVal字典中
    /// - Parameter datas: 传入K线数据模型集合
    /// - Returns: 算法的结果记录到KRChartItem的extVal字典中，返回一个处理后的集合
    func handleAlgorithm(_ datas: [KRChartItem]) -> [KRChartItem]
    
}

//MARK: - Equatable
//public func ==(lhs: KRChartAlgorithm, rhs: KRChartAlgorithm) -> Bool {
...
/**
 常用技术指标算法
 */
public enum KRChartAlgorithm: KRChartAlgorithmProtocol {
    ...
    public func key(_ name: String = "") -> String {
        ...
        case .none:
            return ""
        case .timeline:
            return "\(KRSeriesKey.timeline)_\(name)"
        case let .ma(num):
            return "\(KRSeriesKey.ma)_\(num)_\(name)"
        case let .ema(num):
            return "\(KRSeriesKey.ema)_\(num)_\(name)"
        case .kdj(_, _, _):
            return "\(KRSeriesKey.kdj)_\(name)"
        case .macd(_, _, _):
            return "\(KRSeriesKey.macd)_\(name)"
        case .boll(_, _):
            return "\(KRSeriesKey.boll)_\(name)"
        case .sar(_, _, _):
            return "\(KRSeriesKey.sar)\(name)"
        case let .sam(num):
            return "\(KRSeriesKey.sam)_\(num)_\(name)"
        case let .rsi(num):
            return "\(KRSeriesKey.rsi)_\(num)_\(name)"
        }
    }
    ...
    /**
     ...
     - returns:
     */
    public func handleAlgorithm(_ datas: [KRChartItem]) -> [KRChartItem] {
        switch self {
        case .none:
            return datas
        case .timeline:
...
// MARK: - 《RSI》 处理算法
extension KRChartAlgorithm {

    fileprivate func getAAndB(_ a: Int, _ b: Int, datas: [KRChartItem]) -> [CGFloat] { 
        var tA = a
        if tA < 0 {
            tA = 0
        }
    ...
    fileprivate func handleRSI(_ num: Int, datas: [KRChartItem]) -> [KRChartItem] {
        ...
        for (i, data) in datas.enumerated() {
            ...
            data.extVal["\(self.key(KRSeriesKey.timeline))"] = rsi
        }
        return datas
    }
}
...
extension KRChartAlgorithm {
    ...
    /**
     ...
     - parameter datas: 数据集
     */
    fileprivate func handleTimeline(datas: [KRChartItem]) -> [KRChartItem] {
        for (_, data) in datas.enumerated() {
            data.extVal["\(self.key(KRSeriesKey.timeline))"] = data.closePrice
            data.extVal["\(self.key(KRSeriesKey.volume))"] = data.vol
        }
        return datas
    }
...
extension KRChartAlgorithm {
    ...
    /**
     ...
     - parameter datas: 数据集
     */
    fileprivate func handleMA(_ num: Int, datas: [KRChartItem]) -> [KRChartItem] {
        for (index, data) in datas.enumerated() {
            let value = self.getMAValue(num, index: index, datas: datas)
            data.extVal["\(self.key(KRSeriesKey.timeline))"] = value.0
            data.extVal["\(self.key(KRSeriesKey.volume))"] = value.1
        }
        return datas
    }
    ...
    /**
     ...
     - returns: MA数（价格，交易量）
     */
    fileprivate func getMAValue(_ num: Int, index: Int, datas: [KRChartItem]) -> (CGFloat?, CGFloat?) {
...
extension KRChartAlgorithm {
    ...
    /**
     ...
     - parameter datas: 数据集
     */
    fileprivate func handleEMA(_ num: Int, datas: [KRChartItem]) -> [KRChartItem] {
        ...
        for (index, data) in datas.enumerated() {
            ...
            data.extVal["\(self.key(KRSeriesKey.timeline))"] = ema_price
            data.extVal["\(self.key(KRSeriesKey.volume))"] = ema_vol
...
extension KRChartAlgorithm {
    ...
    /**
     ...
     - returns: 处理好的集合
     */
    fileprivate func handleKDJ(_ p1: Int, p2: Int,p3: Int, datas: [KRChartItem]) -> [KRChartItem] {
    ...
    /**
     ...
     - returns:
     */
    fileprivate func getRSV(_ num: Int, index: Int, datas: [KRChartItem]) -> CGFloat? {
        var rsv: CGFloat = 0
        let c = datas[index].closePrice
...
extension KRChartAlgorithm {
    ...
    /**
     ...
     - parameter datas: 数据集
     */
    fileprivate func handleMACD(_ p1: Int, p2: Int,p3: Int, datas: [KRChartItem]) -> [KRChartItem] {
    ...
    /**
     ...
     - returns: //EMA的成交价和成交量
     */
    fileprivate func getEMA(_ num: Int, index: Int, datas: [KRChartItem]) -> (CGFloat?, CGFloat?) {
        let ema = KRChartAlgorithm.ema(num)
        let data = datas[index]
        let ema_price = data.extVal["\(ema.key(KRSeriesKey.timeline))"]
        let ema_vol = data.extVal["\(ema.key(KRSeriesKey.volume))"]
    ...
    /**
     ...
     - returns: //MA的成交价和成交量
     */
    fileprivate func getMA(_ num: Int, index: Int, datas: [KRChartItem]) -> (CGFloat?, CGFloat?) {
        let ma = KRChartAlgorithm.ma(num)
        let data = datas[index]
        let ma_price = data.extVal["\(ma.key(KRSeriesKey.timeline))"]
        let ma_vol = data.extVal["\(ma.key(KRSeriesKey.volume))"]
...
extension KRChartAlgorithm {
    ...
    ///   - datas: 待处理的数据
    /// - Returns: 处理后的数据
    fileprivate func handleBOLL(_ num: Int, k: Int = 2, datas: [KRChartItem]) -> [KRChartItem] {
    ...
    ///   - datas: 数据集合
    /// - Returns: 结果
    fileprivate func handleBOLLSTD(_ num: Int, index: Int, datas: [KRChartItem]) -> CGFloat {
...
extension KRChartAlgorithm {
    ...
    /// - Returns: 处理后的数据集合
    fileprivate func handleSAR(_ num: Int, minAF: CGFloat, maxAF: CGFloat, datas: [KRChartItem]) -> [KRChartItem] {
        
        var sar: CGFloat = 0, af: CGFloat = minAF, ep: CGFloat = 0
        var pre_data: KRChartItem!
    ...
    ///   - datas: 数据集合
    /// - Returns: 最终值，是否行情翻转
    func getFinalSAR(num: Int, sar: CGFloat, index: Int, isUP: Bool, datas: [KRChartItem]) -> (CGFloat, Bool) {
...
extension KRChartAlgorithm {
    ...
    /**
     ...
     - parameter datas: 数据集
     */
    fileprivate func handleSAM(_ num: Int, datas: [KRChartItem]) -> [KRChartItem] {
        ...
        for (index, data) in datas.enumerated() {
            ...
            if index - max_index == num {
                ...
                for j in max_index...index {
                    datas[j].extVal["\(self.key(KRSeriesKey.timeline))"] = max_vol_price
                    datas[j].extVal["\(self.key(KRSeriesKey.volume))"] = max_vol
                }
            ...
            if index > num - 1 {
                data.extVal["\(self.key(KRSeriesKey.timeline))"] = max_vol_price
                data.extVal["\(self.key(KRSeriesKey.volume))"] = max_vol
                
                //记录填充颜色的最大值
                let priceName = "\(KRSeriesKey.timeline)_BAR"
                let volumeName = "\(KRSeriesKey.volume)_BAR"

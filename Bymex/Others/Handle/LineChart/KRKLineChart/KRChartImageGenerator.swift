...
/// 简单走势图生成器
public class KRChartImageGenerator: NSObject {

    public var values: [(Int, Double)] = [(Int, Double)]()
    public var chartView: KRKLineChartView!
    public var style: KRKLineChartStyle = KRKLineChartStyle.lineIMG
    
    
    /// 创建一个全局单例用于生成图表的截图
    public static let share: KRChartImageGenerator = {
        let generator = KRChartImageGenerator()
        return generator
    }()
    
    public override init() {
        super.init()
        self.chartView = KRKLineChartView(frame: CGRect.zero)
        self.chartView.style = KRKLineChartStyle.lineIMG
...
// MARK: - 自定义风格
extension KRKLineChartStyle {
   
    
    //实现一个点线简单图表用于图片显示
    public static var lineIMG: KRKLineChartStyle {
        ...
        //背景颜色
        style.backgroundColor = UIColor.kr_hex(0xF5F5F5)
        //文字颜色
...
// MARK: - 实现委托方法
extension KRChartImageGenerator: KRKLineChartDelegate {
    
    public func numberOfPointsInKLineChart(chart: KRKLineChartView) -> Int {
        return self.values.count
    }
    
    public func kLineChart(chart: KRKLineChartView, valueForPointAtIndex index: Int) -> KRChartItem {
    ...
    ///
    /// - returns:
    public func widthForYAxisLabelInKLineChart(in chart: KRKLineChartView) -> CGFloat {
        return chart.kYAxisLabelWidth
    }
    
    public func kLineChart(chart: KRKLineChartView, labelOnYAxisForValue value: CGFloat, atIndex index: Int, section: KRSection) -> String {
//        let strValue = value.kr_toString(maxF: section.decimal)
        return ""
    }
    
    public func kLineChart(chart: KRKLineChartView, labelOnXAxisForIndex index: Int) -> String {
        return ""
    }
    
    public func heightForXAxisInKLineChart(in chart: KRKLineChartView) -> CGFloat {
        return 0
    }

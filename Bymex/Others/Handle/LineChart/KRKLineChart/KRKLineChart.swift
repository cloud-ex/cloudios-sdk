...
/**
 ...
 - None:  不处理
 */
public enum KRChartViewScrollPosition {
    case top, end, none
}
...
/// - free: 自由就在显示的点上
/// - onClosePrice: 在收盘价上
public enum KRChartSelectedPosition {
    case free
    case onClosePrice
}
 ...
 *  K线数据源代理
 */
@objc public protocol KRKLineChartDelegate: class {
    ...
    /**
     ...
     - returns:
     */
    @objc optional func kLineChart(chart: KRKLineChartView, labelOnXAxisForIndex index: Int) -> String
    
    /**
     完成绘画图表
     
     */
    @objc optional func didFinishKLineChartRefresh(chart: KRKLineChartView)
    ...
    ///
    /// - returns:
    @objc optional func kLineChart(chart: KRKLineChartView, decimalAt section: Int) -> Int
    ...
    ///
    /// - returns:
    @objc optional func widthForYAxisLabelInKLineChart(in chart: KRKLineChartView) -> CGFloat
    ...
    ///   - index: 点击的位置
    ///   - item: 数据对象
    @objc optional func kLineChart(chart: KRKLineChartView, didSelectAt index: Int, item: KRChartItem)
    ...
    /// - Parameter chart: 图表
    /// - Returns: 返回自定义的高度
    @objc optional func heightForXAxisInKLineChart(in chart: KRKLineChartView) -> CGFloat
    
    
    /// 初始化时的显示范围长度
    ///
    /// - Parameter chart: 图表
    @objc optional func initRangeInKLineChart(in chart: KRKLineChartView) -> Int
    
    
    /// 更新显示范围长度
    ///
    /// - Parameter chart: 图表
    /// - Returns: 返回 KRRangeModle
    @objc optional func updateRangesInKLineChart(in chart: KRKLineChartView) -> KRRangeModle
    ...
    @objc optional func kLineChart(chart: KRKLineChartView, viewOfYAxis yAxis: UILabel, viewOfXAxis: UILabel)
    ...
    ///   - section: 分区的索引位
    /// - Returns: 自定义的View
    @objc optional func kLineChart(chart: KRKLineChartView, viewForHeaderInSection section: Int) -> UIView?
    ...
    ///   - section: 分区的索引位
    /// - Returns: 自定义的View
    @objc optional func kLineChart(chart: KRKLineChartView, titleForHeaderInSection section: KRSection, index: Int, item: KRChartItem) -> NSAttributedString?
    
    
    /// 切换分区用分页方式展示的线组
    ///
    @objc optional func kLineChart(chart: KRKLineChartView, didFlipPageSeries section: KRSection, series: KRSeries, seriesIndex: Int)
}

open class KRKLineChartView: UIView {
    ...
    /// MARK: - 成员变量
    @IBInspectable open var upColor: UIColor = UIColor.ThemekLine.up     //升的颜色
    @IBInspectable open var downColor: UIColor = UIColor.ThemekLine.down     //跌的颜色
    @IBInspectable open var labelFont = UIFont.systemFont(ofSize: 10)
    @IBInspectable open var lineColor: UIColor = UIColor.ThemeView.seperator //线条颜色
    @IBInspectable open var textColor: UIColor = UIColor.ThemeLabel.colorDark //文字颜色
    @IBInspectable open var xAxisPerInterval: Int = 4                        //x轴的间断个数
    
    open var yAxisLabelWidth: CGFloat = 0                    //Y轴的宽度
    open var handlerOfAlgorithms: [KRChartAlgorithmProtocol] = [KRChartAlgorithmProtocol]()
    open var padding: UIEdgeInsets = UIEdgeInsets.zero    //内边距
    open var showYAxisLabel = KRYAxisShowPosition.right      //显示y的位置，默认右边
    open var isInnerYAxis: Bool = false                     // 是否把y坐标内嵌到图表中
    open var selectedPosition: KRChartSelectedPosition = .onClosePrice         //选中显示y值的位置

    @IBOutlet open weak var delegate: KRKLineChartDelegate?             //代理
    
    open var sections = [KRSection]()
    open var selectedIndex: Int = -1                      //选择索引位
    open var scrollToPosition: KRChartViewScrollPosition = .none  //图表刷新后开始显示位置
    ...
    open var showSelection: Bool = true {
        didSet {
            ...
            self.verticalLineView?.backgroundColor = self.verticalLineViewBGColor
            //self.horizontalLineView?.backgroundColor = self.horizontalLineViewBGColor
            self.sightView?.backgroundColor = self.sightViewBGColor
        }
    }
    ...
    open var showSelectBoardView = false {
        didSet {
            if !showSelectBoardView {
                self.tipBoard?.hide()
            } else {
//                self.tipBoard?.isHidden = !showSelectBoardView
            }
        }
    }
    ...
    var verticalLineViewWidth: CGFloat = 0.5
    var plotCount: Int = 0
    /// 可见区域的开始索引位
    var rangeFrom: Int = 0
    /// 可见区域的结束索引位
    var rangeTo: Int = 0
    open var isRightestShow: Bool {
        get {
            return rangeTo == self.plotCount
        }
    }
    /// 显示在可见区域的个数
    open var range: Int = 77
    /// 最大个数
    open var maxRange = 77
    /// 数据足够的时候显示的最小个数（数据足够，但是右边预留一部分不显示）
    open var minRangeWhenRightSpace = Int(77*0.75)
    var borderColor: UIColor = UIColor.ThemeView.border
    open var labelSize = CGSize(width: 40, height: 16)
    
    /// 数据源
    var datas: [KRChartItem] = [KRChartItem]() {
        didSet {
            DispatchQueue.main.async {
                self.latestPriceIndicator.isHidden = self.datas.count == 0
            }
        }
    }
    
    open var selectedBGColor: UIColor = UIColor(white: 0.4, alpha: 1)    //选中点的显示的框背景颜色
    open var selectedBGBorderColor: UIColor = UIColor(white: 0.4, alpha: 1)    //选中点的显示的框边框颜色
    ...
    open var horizontalLineViewBGColor: UIColor = UIColor(white: 1, alpha: 0.1) {
        didSet {
            self.horizontalLineLayer.colors = [self.horizontalLineViewBGColor.withAlphaComponent(0).cgColor,
                                               self.horizontalLineViewBGColor.withAlphaComponent(0.1).cgColor,
                                               self.horizontalLineViewBGColor.withAlphaComponent(0.1).cgColor,
                                               self.horizontalLineViewBGColor.withAlphaComponent(0).cgColor]
        }
    }
    open var sightViewBGColor: UIColor = UIColor.white
    var verticalLineView: UIView?
    var horizontalLineView: UIView?
    lazy var horizontalLineLayer: CAGradientLayer = {
        let layer = CAGradientLayer()
        
        layer.colors = [UIColor(white: 1, alpha: 0).cgColor,
    ...
    /// 用于图表的图层
    var drawLayer: KRShapeLayer = KRShapeLayer()
    
    /// 点线图层
    var chartModelLayer: KRShapeLayer = KRShapeLayer()
    
    /// 图表数据信息显示层，显示每个分区的数值内容
    var chartInfoLayer: KRShapeLayer = KRShapeLayer()
    
    /// pinch
    private weak var pinch: UIPinchGestureRecognizer!
    
    open var style: KRKLineChartStyle! {           //显示样式
        didSet {
            //重新配置样式
            ...
            self.selectedBGBorderColor = self.style.selectedBGBorderColor
            self.verticalLineViewBGColor = self.style.verticalLineViewBGColor
            self.horizontalLineViewBGColor = self.style.horizontalLineViewBGColor
            self.sightViewBGColor = self.style.sightViewBGColor
            ...
            self.showSelectBoardView = self.style.showSelectBoardView
    ...
    public var plotWidth: CGFloat = 10
...
//    convenience init(style: KRKLineChartStyle) {
//        self.init()
//        self.initUI()
//        self.style = style
//    }
    ...
    fileprivate func initUI() {
        ...
        //初始化点击选择的辅助线显示
        self.verticalLineView = UIView(frame: CGRect(x: 0, y: 0, width: verticalLineViewWidth, height: 0))
        self.verticalLineView?.backgroundColor = self.selectedBGColor
        self.verticalLineView?.isHidden = true
        self.addSubview(self.verticalLineView!)
        
        self.horizontalLineView = UIView(frame: CGRect(x: 0, y: 0, width: 0, height: 0))
        self.horizontalLineView?.backgroundColor = UIColor.clear//horizontalLineViewBGColor
        self.horizontalLineView?.isHidden = true
        self.addSubview(self.horizontalLineView!)
        self.horizontalLineView?.layer.addSublayer(self.horizontalLineLayer)
        
        latestPriceIndicator.frame = CGRect.zero
        latestPriceIndicator.btn.addTarget(self, action: #selector(latestPriceIndicatorBtnAction(_:)), for: .touchUpInside)
        self.addSubview(latestPriceIndicator)
        
        //用户点击图表显示当前y轴的实际值
        self.selectedYAxisLabel = UILabel(frame: CGRect.zero)
        self.selectedYAxisLabel?.backgroundColor = self.selectedBGColor
        self.selectedYAxisLabel?.layer.borderWidth = 0.5
        self.selectedYAxisLabel?.layer.borderColor = self.selectedBGBorderColor.cgColor
        ...
        self.selectedXAxisLabel?.layer.borderWidth = 0.5
        self.selectedXAxisLabel?.layer.borderColor = self.selectedBGBorderColor.cgColor
        ...
        self.tipBoard = KRKLineTipBoardView.init(frame: CGRect.init(x: 10, y: 20, width: 130, height: 140))
        self.tipBoard?.backgroundColor = UIColor.ThemeTab.bg
        self.tipBoard?.isHidden = true
        self.addSubview(self.tipBoard!)
     ...
     */
    func getSectionByTouchPoint(_ point: CGPoint) -> (Int, KRSection?) {
    ...
    ///
    /// - Returns:
    func getSecionWhichShowXAxis() -> KRSection {
        let visiableSection = self.sections.filter { !$0.hidden }
        var showSection: KRSection?
...
//        self.sightView?.isHidden = !self.showSelection
        ...
        //每个点的间隔宽度
        let plotWidth = self.plotWidth//(section!.frame.size.width - section!.padding.left - section!.padding.right) / CGFloat(self.rangeTo - self.rangeFrom)
        ...
        for i in self.rangeFrom...self.rangeTo - 1 {
            ...
            if ixs <= point.x && point.x < ixe {
                guard datas.count > selectedIndex else { return }
                ...
                //显示辅助线
                self.horizontalLineView?.frame = CGRect(x: hx-(plotWidth-2*self.pinch.scale)/2, y: hy, width: plotWidth-2*self.pinch.scale, height: hheight)
                //                self.horizontalLineView?.isHidden = false
                self.horizontalLineLayer.frame = self.horizontalLineView?.bounds ?? CGRect.zero
                ...
                case .onClosePrice:
                    if let series = section?.getSeries(key: KRSeriesKey.candle), !series.hidden {
                        yVal = item.closePrice          //获取收盘价作为实际值
                    } else if let series = section?.getSeries(key: KRSeriesKey.timeline), !series.hidden {
                        yVal = item.closePrice          //获取收盘价作为实际值
                    } else if let series = section?.getSeries(key: KRSeriesKey.volume), !series.hidden {
                ...
                //显示辅助线
                self.verticalLineView?.frame = CGRect(x: vx, y: vy - self.verticalLineViewWidth / 2, width: hwidth, height: self.verticalLineViewWidth)
                ...
                let time = Date.kr_getTimeByStamp(item.time, format: "yyyy-MM-dd HH:mm") //显示实际值
                let size = time.kr_sizeWithConstrained(self.labelFont)
                ...
                if i < datas.count && section?.name != "timeline" && self.showSelectBoardView {
                    self.tipBoard?.isHidden = false
                    let fm = DateFormatter()
                    fm.dateFormat = "yy-MM-dd HH:mm"
                    let datetimeString = fm.string(from: Date.init(timeIntervalSince1970: TimeInterval(datas[i].time)))
                    let tipDatas = KRKLineTipBoardView.Datas.init(open: "\(Double(datas[i].openPrice))",
                                                               close: "\(Double(datas[i].closePrice))",
                                                               high: "\(Double(datas[i].highPrice))",
                                                               low: "\(Double(datas[i].lowPrice))",
                                                               time: datetimeString,
                                                               riseDrop: "",
                                                               percent: "",
                                                               vol: "\(Double(datas[i].vol))")
                    self.tipBoard?.datas = tipDatas
                    self.tipBoard?.show(tipPoint: CGPoint.init(x: point.x, y: 20), klineViewWidth: self.bounds.width)
                } else {
                    self.tipBoard?.isHidden = true
                }
...
extension KRKLineChartView {
        ...
        if self.initChart() {
            ...
            self.buildSections {
                ...
                if let titleView = self.delegate?.kLineChart?(chart: self, viewForHeaderInSection: index) {
                    ...
                    if self.datas.count > selectedIndex, let titleString = self.delegate?.kLineChart?(chart: self,
                                                                   titleForHeaderInSection: section,
                ...
                //绘制最新价格横虚线
                guard section.key == .price else {
                    return
                }
                self.bringSubviewToFront(latestPriceIndicator)
                self.bringSubviewToFront(self.tipBoard!)
                
                latestPriceIndicator.frameHeight = 24
                var y = section.frame.origin.y
                
                if self.datas.last?.closePrice ?? 0 < section.yAxis.min {
                    y = section.getLocalY(section.yAxis.min)+8
                }else if self.datas.last?.closePrice ?? 0 > section.yAxis.max {
                    y = section.getLocalY(section.yAxis.max)-8
                }else{
                    y = section.getLocalY(self.datas.last?.closePrice ?? 0)
                }
                
                //此时y对应最新价格指示器的centerY，它的y还需要减去自身高度一半
                y -= latestPriceIndicator.frameHeight/2
                self.latestPriceIndicator.reDraw(CGRect.init(x: section.frame.origin.x,
                                                             y: y,
                                                             width: section.frame.width,
                                                             height: self.latestPriceIndicator.frameHeight),
                                                 latestPrice: "\(Double(self.datas.last?.closePrice ?? 0))",
                                                 isRightestShow: self.isRightestShow,
                                                 lastCandleLayerXAddW: (section.lastCandleLayerXAddW ?? 0)+4)
            }
    ...
    */
    ...
    fileprivate func initChart() -> Bool {
        ...
        if plotCount > 0 {
            ...
            if self.scrollToPosition == .top {
                ...
                if self.rangeTo - self.range > 0 {          //如果尽头 - 默认显示数大于0
                    ...
                    self.rangeFrom = self.rangeTo - min(range, minRangeWhenRightSpace)   //计算开始的显示的位置
                } else {
                    self.rangeFrom = 0
                }
                self.selectedIndex = -1
        ...
        let backgroundLayer = KRShapeLayer()
...
//        context?.fill (CGRect (x: 0, y: 0, width: self.bounds.size.width,height: self.bounds.size.height))
    ...
    fileprivate func buildSections(
        _ complete:(_ section: KRSection, _ index: Int) -> Void) {
        //计算实际的显示高度和宽度
    ...
    /**
     ...
     - parameter width:   总宽度
     */
    fileprivate func drawXAxis(_ section: KRSection) -> [(CGRect, String)] {
        
        var xAxisToDraw = [(CGRect, String)]()
        
        let xAxis = KRShapeLayer()
        ...
        for i in stride(from: self.rangeFrom, to: self.rangeTo, by: xTickInterval) {
            ...
            var textSize = xLabel.kr_sizeWithConstrained(self.labelFont)
...
//            let referenceLayer = KRShapeLayer()
...
//            }
        ...
        for index in 0..<section.xAxis.tickInterval {
            ...
            //需要画x轴上的辅助线
            if showXAxisReference {
                referencePath.move(to: CGPoint(x: secWidth/CGFloat(section.xAxis.tickInterval)*CGFloat(index), y: section.frame.minY))
                referencePath.addLine(to: CGPoint(x: secWidth/CGFloat(section.xAxis.tickInterval)*CGFloat(index), y: section.frame.maxY))
                referenceLayer.path = referencePath.cgPath
                xAxis.addSublayer(referenceLayer)
            }
        }
    ...
    ///   - section: 哪个分区绘制
    ///   - xAxisToDraw: 待绘制的内容
    fileprivate func drawXAxisLabel(_ section: KRSection, xAxisToDraw: [(CGRect, String)]) {
    ...
    /**
     ...
     - parameter section:
     */
    fileprivate func drawSection(_ section: KRSection) {
        
        //画分区的背景
        let sectionPath = UIBezierPath(rect: section.frame)
        let sectionLayer = KRShapeLayer()
...
//        self.drawLayer.addSublayer(sectionLayer)
        self.drawLayer.insertSublayer(sectionLayer, at: 0)
        
        //渐变背景
        section.drawGradientBG(sectionLayer)
        section.drawLogo(sectionLayer)
    ...
    /**
     初始化分区上各个线的Y轴
     */
    fileprivate func initYAxis(_ section: KRSection) {
     ...
     */
    fileprivate func drawYAxis(_ section: KRSection) -> [(CGRect, String)] {
...
//        }
    ...
    /**
     ...
     - parameter section:
     */
    func drawChart(_ section: KRSection) {
        if section.paging {
            ...
            if section.selectedIndex < section.series.count {
                ...
                section.lastCandleLayerXAddW = serie.lastCandleLayerXAddW
            } else {
                print("--")
            }
        } else {
            ...
            for serie in section.series {
                let seriesLayer = self.drawSerie(serie)
                section.sectionLayer.addSublayer(seriesLayer)
                if serie.lastCandleLayerXAddW != nil {
                    lastCandleLayerXAddW = serie.lastCandleLayerXAddW
                }
            }
            section.lastCandleLayerXAddW = lastCandleLayerXAddW
        }
    ...
    /**
     绘制图表分区上的系列点先
     */
    func drawSerie(_ serie: KRSeries) -> KRShapeLayer {
        if !serie.hidden {
            //循环画出每个模型的线
            for (index, model) in serie.chartModels.enumerated() {
                if index+1 == serie.chartModels.count, model.key == KRSeriesKey.candle {
                    UIView.animate(withDuration: 0.35) {
                        let serieLayer = model.drawSerie(self.rangeFrom, endIndex: self.rangeTo)
                        serie.seriesLayer.addSublayer(serieLayer)
                        serie.lastCandleLayerXAddW = (model as? KRCandleModel)?.lastCandleLayerXAddW
                    }
                }else{
                    let serieLayer = model.drawSerie(self.rangeFrom, endIndex: self.rangeTo)
                    serie.seriesLayer.addSublayer(serieLayer)
...
extension KRKLineChartView {
    ...
    /**
     刷新视图
     */
    public func reloadData(toPosition: KRChartViewScrollPosition = .none, resetData: Bool = true) {
    ...
    public func refreshLatestInfo(animated: Bool = true) {
        guard self.plotCount > 0 else {
            print(#function+"--没有数据，plotCount==0")
            return
        }
        guard self.datas.count > 0 else {
            return
        }
        guard let item = self.delegate?.kLineChart(chart: self, valueForPointAtIndex: self.datas.count-1) else {
            return
        }
        
        guard let priceSection = self.sections.first(where: { (section) -> Bool in
            return section.key == .price
        }) else {
            return
        }
        
        guard item.closePrice >= priceSection.yAxis.min, item.closePrice <= priceSection.yAxis.max else {
            self.reloadData(resetData: true)
            return
        }
    ...
    ///
    /// - Parameter style: 新风格
    public func resetStyle(style: KRKLineChartStyle) {
        self.style = style
        self.showSelection = false
        self.showSelectBoardView = false
        self.reloadData()
    }
...
//        self.drawLayerView()
    ...
    /**
     通过key隐藏或显示分区
     */
    public func setSection(hidden: Bool, byKey key: KRSection.Key) {
        for section in self.sections {
            //副图才能隐藏
...
//        self.drawLayerView()
    ...
    /// 缩放图表
    private func zoomChart(scale: CGFloat) {
        guard let ranges = self.delegate?.updateRangesInKLineChart?(in: self) else {
            return
        }
        self.range = ranges.range
        self.maxRange = ranges.maxRange
        self.minRangeWhenRightSpace = ranges.minRangeWhenRightSpace
        
        //该分区每个点的间隔宽度
        let plotWidth = self.plotWidth
        
        //根据放大比例计算一个新的列宽   (plotWidth最大24pt，最小4pt)
        let newPlotWidth = max(min(plotWidth * scale, 24), 4)
        guard self.plotWidth != newPlotWidth else {
            return
        }
        self.plotWidth = newPlotWidth
        self.sections.forEach { (aCHSection) in
            aCHSection.series.forEach { (aCHSeries) in
                aCHSeries.chartModels.forEach { (aKRChartModel) in
                    aKRChartModel.plotWidth = newPlotWidth
                }
            }
        }
    ...
    public func moveChart(by interval: Int, direction: Bool) {
        guard interval > 0 else {//有移动间隔才移动
            //self.range = self.rangeTo - self.rangeFrom
            return
        }
        guard  self.plotCount > (self.rangeTo-self.rangeFrom) else {
            //self.range = self.rangeTo - self.rangeFrom
            return
        }
        
        switch direction {
        case true://单指向右拖，往后查看数据
            if self.rangeFrom - interval >= 0 {
                self.rangeFrom -= interval
                //计算需要增加蜡烛的数量
                let needAddCount = min(max(maxRange-range, 0), interval)
                if needAddCount > 0 {
                    self.rangeTo -= interval-needAddCount
                }else{
                    self.rangeTo   -= interval
                }
//                self.rangeTo   -= (needAddCount > 0 ? (interval-needAddCount) : interval)
            } else {
                self.rangeFrom = 0
                self.rangeTo -= self.rangeFrom
            }
        case false://单指向左拖，往前查看数据
            guard self.rangeTo <= self.plotCount else {
                return
            }
            //计算需要减少蜡烛的数量
            let needSubCount = min(max(range-minRangeWhenRightSpace, 0), interval)
            if self.rangeTo >= self.plotCount {//右边最后一条数据已经显示
                self.rangeFrom += needSubCount
                self.rangeTo  = self.plotCount
            }else{
                self.rangeFrom += interval
                self.rangeTo += interval
    ...
    ///   - series: 线段
    ///   - section: 分区位置
    open func addSeries(_ series: KRSeries, inSection section: Int) {
        guard let section = self.sections[safe: section] else {
            return
        }
...
// MARK: - 手势操作
extension KRKLineChartView: UIGestureRecognizerDelegate {
...
//                if self?.rangeFrom == 0 || self?.rangeTo == self?.plotCount{
                ...
                if self?.rangeFrom == 0 || range == self?.plotCount || range == self?.minRangeWhenRightSpace {
                    return
                }
...
//            print("scale = \(scale)")
        ...
        */
        self.zoomChart(scale: sender.scale)
        sender.scale = 1    //恢复比例
...
//MARK: - Actions
extension KRKLineChartView {
    @objc func latestPriceIndicatorBtnAction(_ sender: UIButton) {
        self.reloadData(toPosition: .end, resetData: false)
    }
}

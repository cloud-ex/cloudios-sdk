...
public struct LockConfig: PatternLockViewConfig {
    ...
    public var initGridClosure: (Matrix) -> (PatternLockGrid) = {_ in
        ...
        let outerFillColorStatus = GridPropertyStatus<UIColor>(normal: nil, connect: UIColor.ThemeLabel.colorHighlight.withAlphaComponent(0.3), error: UIColor.red.withAlphaComponent(0.3))
        ...
        let innerFillColorStatus = GridPropertyStatus<UIColor>(normal: UIColor.ThemeLabel.colorHighlight, connect: UIColor.ThemeLabel.colorHighlight, error: UIColor.red)

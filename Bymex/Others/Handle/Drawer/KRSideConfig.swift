...
public enum KRSideAnimationType:Int {
    case zoom //缩放
    case translationPush //横向平移
    case translationMask //盖在最上面横向平移
}

public enum KRSideDirection:Int {
    case left  // 从左边出来
    case right // 从右边出来
}

public class KRSideConfig: NSObject {
    ...
    public var sideRelative: CGFloat! = 300/375 {
        didSet {
    ...
    /// 侧边来出来的的方向 默认从左边出来
    public var direction:KRSideDirection! = .left
    
    /// 侧边来出来的动画方式
    public var animationType:KRSideAnimationType! = .translationPush {
        didSet {
            if animationType == .zoom  {sideRelative = 1.0}
        }
    }

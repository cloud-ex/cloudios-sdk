...
let KRSideTapNotification = "KRSideTapNotification"
let KRSidePanNotification = "KRSidePanNotification"
...
extension UIViewController {
    ...
    public func gy_showSide(configuration:(KRSideConfig)->(), viewController:UIViewController) {
        
        let config = KRSideConfig()
        configuration(config)
        var delegate = objc_getAssociatedObject(self, &showControlelrTransitioningDelegateKey)
        if delegate == nil {
            delegate = KRSideTransitioningDelegate(config:config)
            ...
            let d = delegate as! KRSideTransitioningDelegate
            d.config = config
        }
        let d = delegate as! KRSideTransitioningDelegate
        let dismissalInteractiveTransition = KRSidePercentInteractiveTransition(showType: .hidden, viewController:viewController, config: config)
    ...
    public func gy_registGestureShowSide(completeShowGesture:@escaping (KRSideDirection)->()) {
        let  delegate = KRSideTransitioningDelegate(config: nil)
        let presentationInteractiveTransition = KRSidePercentInteractiveTransition(showType: .show, viewController: nil, config: nil)

...
enum KRSideShowType:Int {
    case show,hidden
}

class KRSideAnimatedTransitioning: NSObject,UIViewControllerAnimatedTransitioning {
    
    var type:KRSideShowType!
    var config:KRSideConfig!
    
    init(showType: KRSideShowType,config: KRSideConfig) {
        super.init()
        type = showType
        self.config = config
    }
    ...
    func animateTransitionShowType(transitionContext: UIViewControllerContextTransitioning) {
        ...
        var mask:KRSideMaskView?
        for view in toController.view.subviews {
            if view.isKind(of: KRSideMaskView.classForCoder()){
                mask = view as? KRSideMaskView
                break
            }
        }
        if mask == nil {
    ...
    func animateTransitionHiddenType(transitionContext: UIViewControllerContextTransitioning) {
        ...
        let flag: CGFloat! = config.direction == KRSideDirection.left ? -1.0 : 1.0
        var fromTransform:CGAffineTransform = CGAffineTransform.init(translationX: flag * width, y: 0)

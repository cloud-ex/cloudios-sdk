...
class KRVerifyCodeView: UIView {
    ...
    /// 输入框
    lazy var textFiled: KRVerifyTextView = {
        let textFiled = KRVerifyTextView()
    ...
    func initSubviews() {
        ...
        // 每个验证码框宽度
        let itemWidth: CGFloat = (SCREEN_WIDTH - 40 - padding * 2 - spacing * (CGFloat(inputTextNum) - 1)) / CGFloat(inputTextNum)
        for i in 0..<inputTextNum {
            let codeNumView = KRVerifyNumView()
...
extension KRVerifyCodeView: UITextFieldDelegate {

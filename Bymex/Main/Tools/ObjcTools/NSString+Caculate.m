...
- (NSString *)toPercentString:(int)pointCount {
    ...
    if (pointCount < 0) {
        ...
        format = [NSString stringWithFormat:@"%%.%df",pointCount+1];
    }
    double l = self.doubleValue;
    l = l*100;
    NSString *res = [NSString stringWithFormat:format,l];
    res = [NSString stringWithFormat:@"%@%%",[res substringToIndex:res.length-1]];
    return res;
}

...
extension UIButton  {
     ...
     */
    public final func extSetTitles(_ titles : [String] , titleColors : [UIColor] , controlStates : [UIControl.State] , target : AnyObject  , selector : Selector , fontSize : CGFloat = 18.0 , backgroundColor : UIColor = UIColor.ThemeLabel.colorLite , tag : Int = 0  ){
    ...
    public final func extSetTitle(_ title : String , _ titleFont : CGFloat , _ titleNormalColor : UIColor ,_ titleSelectedColor : UIColor){
        self.setTitle(title, for: state)
        self.titleLabel?.font = UIFont.systemFont(ofSize: titleFont)
    ...
    /**
     ...
     - parameter tag:             tag值 默认0 可不传
     */
    public final func extSetTitle(_ title : String , titleColor : UIColor , fontSize : CGFloat = 16.0){
    ...
    public func countdown(_ num : Int ,unit : String = "s" ,defaultValue : String = "" , complete : (() -> ())? = nil){
        if num >= 0{
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                self.setTitle(defaultValue + "\(num)" + unit, for: UIControl.State.normal)
...
//        [btn setImageEdgeInsets:UIEdgeInsetsMake(0, btn.titleLabel.bounds.size.width, 0, -btn.titleLabel.bounds.size.width)];
    ...
    public convenience init(text: String?,font: UIFont?,textColor: UIColor?,boardWidth:CGFloat=0.5,boardColor:UIColor=UIColor.ThemeView.border) {
        self.init()
        self.setTitle(text, for: .normal)
        self.setTitleColor(textColor, for: .normal)
        self.titleLabel?.font = font
        self.extSetBorderWidth(boardWidth, color: boardColor)
    }
}
